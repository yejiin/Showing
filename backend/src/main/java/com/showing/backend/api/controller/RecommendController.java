...
public class RecommendController {
    ...
    @ApiOperation(value = "사용자 추천 공연 목록 조회", notes = "사용자가 별점을 4점 이상 평가한 공연들에 대해 비슷한 공연 목록을 랜덤으로 15개 조회합니다.")
    @ApiResponses({@ApiResponse(code = 200, message = GET_RECOMMEND),
            @ApiResponse(code = 400, message = "Invalid Input 오류", response = ErrorResponse.class),
            @ApiResponse(code = 401, message = "권한 인증 오류", response = ErrorResponse.class),
            @ApiResponse(code = 404, message = "Not Found 오류", response = ErrorResponse.class),
    ...
    @ApiOperation(value = "사용자 선호 배우 출연 공연 목록 조회", notes = "사용자가 선호하는 상위 5명 배우 중 한명의 출연 공연 목록을 조회합니다.")
    @ApiResponses({@ApiResponse(code = 200, message = GET_RECOMMEND),
            @ApiResponse(code = 400, message = "Invalid Input 오류", response = ErrorResponse.class),
            @ApiResponse(code = 401, message = "권한 인증 오류", response = ErrorResponse.class),
            @ApiResponse(code = 404, message = "Not Found 오류", response = ErrorResponse.class),
            @ApiResponse(code = 500, message = "서버 에러", response = ErrorResponse.class)})
    @GetMapping("/users/{userId}/favorite-actor")
    public ResponseEntity<BaseResponseBody> getRecommendPerformanceListByUserFavoriteActor(@PathVariable Long userId) {
        // userId 유효성 검사
        if (userId == null || !Objects.equals(userId, JwtUtil.getCurrentId().orElse(null))) {
            throw new AccessDeniedException(ErrorCode.ACCESS_DENIED.getMessage());
        }

        return ResponseEntity.ok(BaseResponseBody.of(HttpStatus.OK, GET_RECOMMEND, recommendService.getFavoriteActorPerformanceListByUser(userId)));
    }
