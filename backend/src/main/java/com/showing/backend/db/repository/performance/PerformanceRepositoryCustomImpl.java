...
public class PerformanceRepositoryCustomImpl implements PerformanceRepositoryCustom {
        ...
        public List<PerformanceRes> findByTypeAndStarPointAvg(int performanceType) {
                return jpaQueryFactory.select(Projections.constructor(PerformanceRes.class,
                                qPerformance.id, qPerformance.performanceName, qPerformance.performanceType,
                                qPerformance.lastSeasonId,
                                qPerformance.performanceImage, qSeason.startDate, qSeason.endDate, qSeason.proceedFlag,
                                qRanking.averageRating))
        ...
        public List<SearchRes> findByPerformanceNameContaining(String keyword) {
                return jpaQueryFactory.select(Projections.constructor(SearchRes.class,
                                ...
                                .orderBy(Expressions
                                                .stringTemplate("DATE_FORMAT({0},{1})", qSeason.startDate, "%Y-%m-%d")
                                                .desc())
                                .fetch();
        }
